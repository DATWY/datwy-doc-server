...
async function testNoSpacers() {
  ...
  await page.evaluate(async () => {
    // 1. Remove all spacers inside [data-page-index]
    document.querySelectorAll('[data-page-index] > div:not(.pf)').forEach(el => el.remove());
    ...
    // Again remove any newly injected spacers
    document.querySelectorAll('[data-page-index] > div:not(.pf)').forEach(el => el.remove());
  ...
  const out = './tmp/pdf_cache/exact_42_pages.pdf';
  fs.writeFileSync(out, pdf);

  const str = pdf.toString('latin1');
  const matches = str.match(/\/Type\s*\/Page\b/g) || [];
  console.log('EXACT PDF RESULT -> Pages:', matches.length, 'Size:', pdf.length);

  await browser.close();
}

testNoSpacers().catch(console.error);
